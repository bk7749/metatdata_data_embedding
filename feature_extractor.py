...
#while start<len(ts):
	for logTick in logRange[1:]:
		...
		sumList.append(sum(fftResult[start:end]))
	maxSum = max(sumList)
...
def get_fluxpercentilemid20(npts):
...
#feats = add_feature(feats, get_fft, normalizedValues)
	feats = add_feature(feats, get_dominating_freq_range, ts)
...
#feats = add_feature(feats, get_lineartrend, normalizedNpts)
	...
	feats = add_feature(feats, get_oom, normalizedNpts)
#feats = add_feature(feats, get_fft, normalizedValues)
